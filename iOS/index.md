# How to ___ in iOS

## UI

- [如何调整 text field 的边缘间距](./adjust-textfield-margin.md)
- [如何调整 text view 的边缘间距](./adjust-textview-margin.md)
- [如何在 text view 中添加 Placeholder](./add-placeholder-textview.md)
- [如何设置 Tab Bar 中的图标居中显示](./tab-bar-icon-center.md)
- [如何正确地在主线程中调度 UI 任务](./dispatch-on-right-queue.md)

## Foundation

- [如何获取 iOS App 的沙盒路径](./sandbox-paths.md)
- [向对象添加或读取了不存在的 Key, 如何挽救](./save-undefined-key.md)

### Runtime 运用

- [如何在 category 中关联对象](./associate-object-category.md)
- [如何实现 method swizzle ?](./method-swizzle.md)
- [如何向类中动态添加方法？](./add-method.md)

